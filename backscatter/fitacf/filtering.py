...
class Filtering(object):
    ...
    def mark_bad_samples(raw_data):
        ...
        pulses_stereo = []
        if offset != 0 and (channel == 1 or channel == 2):  # REVIEW # - consistency with parentheses
            for pulse in pulses:
                if channel == 1:
        ...
        for pulse_us in pulses_in_us:
            ...
            t1 = pulse_us - txpl / 2  # REVIEW #29 - what is 2, 3 and 100?
            t2 = t1 + 3 * txpl / 2 + 100
            ...
            # it as a bad sample
            while ((ts >= t1) and (ts <= t2)):  # REVIEW # - consistency with paretheses
    ...
    def filter_low_pwr_lags(raw_data, range_list):
        ...
        for range_obj in range_list:
            ...
            for idx, (log_pwr, alpha_2) in enumerate(zip(range_obj.pwrs.log_pwrs,
