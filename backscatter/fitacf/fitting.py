...
class ACFFitting(object):
    ...
    elevation, as well the methods to calculate sigmas and unwrap phase # REVIEW #7 What are sigmas?
    ...
    def acf_pwr_fitting(range_list):
        ...
        for range_obj in range_list:
            ...
            if len(log_pwrs) != len(sigmas) != len(t):  # REVIEW #0 This 3 value if statement doesn't work... you'll need to use 'or' (also for better readability)
                error_msg = """The lengths of the data point arrays don't agree in
            ...
            range_obj.linear_pwr_fit_err = two_param_line_fit(t, log_pwrs,
                                                              log_corrected_sigmas, num_points)
            range_obj.quadratic_pwr_fit_err = quad_fit(t, log_pwrs,
                                                       log_corrected_sigmas, num_points)
    ...
    def acf_phase_fitting(range_list):
        ...
        for range_obj in range_list:
            ...
            if len(phase_values) != len(sigma_values) != len(t_values): # REVIEW #0 This 3 value if statement doesn't work (as we tested with python 2.7)
                error_msg = """The lengths of the data point arrays don't agree in
            ...
            range_obj.phase_fit = one_param_line_fit(t_values, phase_values,
                                                     sigma_values, num_points)
    ...
    def xcf_phase_fitting(range_list):
        ...
        for range_obj in range_list:
            ...
            if len(elev_values) != len(sigma_values) != len(t_values): # REVIEW #0 This 3 value if statement doesn't work (as we tested with python 2.7)
                error_msg = """The lengths of the data point arrays don't agree in
    ...
    @staticmethod
    def calculate_phase_and_elev_sigmas(range_list, raw_data): # REVIEW #7 Can you reference paper for the equations used?
        ...
        for range_obj in range_list:
            ...
            pwrs = range_obj.pwrs  # REVIEW #42 Not used
            elevs = range_obj.elevs

            phase_inverse_alpha_2 = 1/phases.alpha_2
            elev_inverse_alpha_2 = 1/elevs.alpha_2

            # phase and elevation have same t values  # REVIEW #7 Where is 't' defined/described in documentation?
            pwr_values = np.exp(-1 * math.fabs(range_obj.linear_pwr_fit.b) * phases.t) # REVIEW #7 What is 'b'?
            ...
            if np.isnan(phase_sigmas).any() or np.isinf(phase_sigmas).any():
                ...
                error_string.format(range_obj.range_number)  # REVIEW #15 Does this mean something should be done about the fit?
            ...
            same as lag 1 sigma"""  # REVIEW #40 should this be a comment? also we don't understand what this is for
    ...
    @staticmethod
    def acf_phase_unwrap(range_list, raw_data): # REVIEW #42, raw_data is not being used
        ...
        for range_obj in range_list:
            ...
            phase_values = phases.phases  # REVIEW #26, we have a paper from Pasha that uses phi instead of the term 'phases'. Since you use 'sigma' for the error, maybe you can use 'phi' for the phase as well
            sigma_values = phases.sigmas
            t_values = phases.t  # REVIEW #26 we think it might be 'time' or 'tau', not sure though.
            ...
            while not orig_phase_iterator.finished:
                ...
                t_diff = t_curr - t_prev  # REVIEW #26 - tdiff eh? I assume not related to lil' Tyler Diff?

                if np.fabs(phase_diff) < math.pi:
                    slope_numerator = slope_numerator + phase_diff/(sigma_bar**2)/t_diff  # REVIEW #3 Couldn't really find info on how this works
            ...
            if total_2pi_corrections > 0:
                ...
                S_xx, S_xy = 0.0, 0.0  # REVIEW #40 pycharm complains about the upper case variable names here
                while not corr_phase_iterator.finished:
                    phase = corr_phase_iterator[0]
                    sigma = corr_phase_iterator[1]
                    t = corr_phase_iterator[2]
                    if sigma > 0.0:  # REVIEW #2 When is sigma not positive? When it has not been calculated or something?
                ...
                if (orig_slope_err > corr_slope_err):  # REVIEW #40 pycharm complains about parenthesis
                    range_obj.phases.set_phases(new_phases)

    @staticmethod
    def xcf_phase_unwrap(range_list):
        """Unwraps the ACF phase data to be able to fit a straight line  # REVIEW #7 should this say 'XCF phase data'?
        ...
        for range_obj in range_list:
            ...
            slope_estimate = np.divide(S_xy, S_xx) # REVIEW #0 You used np.divide here, but are S_xx and S_xy not single values? Above you just used '/' operator in acf_phase_unwrap(...)
            new_phases = phase_correction(slope_estimate, new_phases, t_values)[0]

            range_obj.elevs.set_phases(new_phases)  # REVIEW #0 not calculating sigma like you do above and comparing with the original phase errors?
    ...
    def phase_correction(slope_estimate, phase_values, t_values):
        ...
        :param phase_values: a particular phase to correct
        :param t_values: a value in time for a particular phase point
        :returns: phase shifted by number 2*pi corrections, total 2pi corrections.  # REVIEW #5 What are the units? We assume radians

        """

        phase_predicted = slope_estimate * t_values

        # I add a rounding here so that if there is inexact division of pi/2pi then  # REVIEW #3 maybe just explain why you want to round all the way to a whole number?
        # .49999999... gets rounded up first.
        phase_diff = np.around((phase_predicted - phase_values)/(2 * math.pi), decimals=5)  # REVIEW #0 would ceil(...) be better suited for this?
