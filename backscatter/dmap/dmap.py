...
class RawDmapRead(object):
    ...
    def parse_record(self):
        ...
        if (self.cursor - bytes_already_read) != size:
            message = "PARSE RECORD: Bytes read {0} does not match " \
                    "the records size field {1}".format(self.cursor-bytes_already_read, size)
            raise DmapDataError(message)
    ...
    def parse_array(self, record_size):
        ...
        # total_elements = reduce(lambda x,y: x*y,dimensions) #REVIEW #33
        total_elements = 1
        ...
        if LOGGING:
            with open("logfile.txt", 'a') as f:
                f.write("PARSE ARRAY: total elements "
    ...
    def read_data(self, data_type_fmt):  # REVIEW #1 We see that you pass an 'i' to this function from initial_data_integrity_check and we don't see what 'i' means. Can you add documentation to describe the possible data_type_fmt values?
        ...
        if data_type_fmt is DMAP:
            ...
            while self.dmap_bytearr[self.cursor + byte_counter] is not 0:
                # print(self.dmap_bytearr[self.cursor + byte_counter]) #REVIEW #33
    ...
    def get_num_bytes(self, data_type_fmt):
        ...
        return {
            ...
            'f': 4,
            'd': 8,
        }.get(data_type_fmt, 0)  # REVIEW #29 what is 0?
...
class RawDmapWrite(object):
    ...
    def __init__(self, data_dicts, file_path, ud_types={}):
        ...
        # print(self.dmap_bytearr) #REVIEW #33
            ...
            if isinstance(v, (list, np.ndarray)):
                ...
                if isinstance(v, np.ndarray):
                    if data_type_fmt == 'c' and v.dtype != 'S1':
                        data = np.asarray([chr(x) for x in v], dtype='c')
                    # elif data_type_fmt == 's':  #REVIEW #33
                    #   data = np.asarray(v,dtype=object)
                    else:
                        data = np.asarray(v, dtype=data_type_fmt)
                ...
                array = RawDmapArray(k, dmap_type, data_type_fmt,
                                     mode, dimension, arr_dimensions, data)
                record.add_array(array)

            else:
                dmap_type = self.convert_fmt_to_dmap_type(data_type_fmt)
                mode = 6  # REVIEW #29 Why 6 now?
    ...
    def dmap_record_to_bytes(self, record):
        ...
        code = 65537  # REVIEW #29 What does this number mean?
    ...
    def dmap_scaler_to_bytes(self, scaler):
        ...
        if data_type_fmt == 's':
            ...
            # data_bytes = scaler.get_data().encode('utf-8') + chr(0) # REVIEW #33
    ...
    def dmap_array_to_bytes(self, array):
        ...
        data_type_fmt = array.get_datatype_fmt()  # REVIEW #26 is it suppose to be seld.data_type_fmt?
    ...
    for dd in data_dicts:
        for k, v in dd.iteritems():
            if k not in ud_types:
                message = "DICTS_TO_FILE: A supplied dictionary "
                "contains extra field {0}".format(k)
                raise DmapDataError(message)
